...
export function startMonitorApiServer(port = 8090) {
  ...
  // GET /api/scanner/hotlist - Get scanner hotlist (from signals.json file)
  app.get("/api/scanner/hotlist", async (req, res) => {
    try {
      ...
      // Try to load signals from signals.json file (generated by scanner)
      let hotlist = [];
      const signalsFile = path.join(CONFIG.rootDir, 'data', 'signals.json');
      ...
      try {
        if (fs.existsSync(signalsFile)) {
          ...
          if (data.signals && Array.isArray(data.signals)) {
            ...
            hotlist = data.signals.map(signal => ({
              symbol: signal.symbol,
              side: signal.direction,
              finalScore: parseFloat(signal.confidence) || 100,
              baseScore: parseFloat(signal.confidence) || 100,
              signal: 'ARM',
              tier: 'NORMAL',
              entry: signal.entry,
              ...
              imbalance: signal.live?.imbalance || 0,
              spread: signal.live?.spread || 0,
              price: signal.live?.price || 0,
              updatedAt: signal.timestamp || new Date().toISOString()
            }));
          }
        }
      } catch (fileError) {
        // File not found or parse error - will use empty hotlist
      }

      // Filter by side
      if (side === 'LONG') {
        hotlist = hotlist.filter(h => h.side === 'LONG');
      } else if (side === 'SHORT') {
        hotlist = hotlist.filter(h => h.side === 'SHORT');
      }

      // Filter by minScore
      hotlist = hotlist.filter(h => h.finalScore >= minScore);

      // Sort by score DESC
      hotlist.sort((a, b) => b.finalScore - a.finalScore);
